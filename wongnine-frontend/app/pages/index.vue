...
const { currentUser, logout, apiFetch, resendVerification } = useAuth()
...
const handleResendVerification = async () => {
  try {
    ...
    alert('ส่งอีเมลยืนยันใหม่แล้ว กรุณาตรวจสอบกล่องจดหมาย')
  } catch (err) {
    alert(err.data?.message || 'ส่งอีเมลไม่สำเร็จ กรุณาลองใหม่')
  }
}
...
const submitNewRestaurantToApi = async (payload, callbacks) => {
  try {
    ...
    showError('เกิดข้อผิดพลาดในการเพิ่มร้านอาหาร')
    callbacks?.onError?.()
  }
}
...
<template>
  <div class="flex flex-col md:flex-row h-screen w-full overflow-hidden bg-[#F7F8F5] font-['Prompt']">
    ...
    >
      ...
      <div
        v-if="currentUser && !currentUser.isVerified"
        class="mx-5 mt-3 p-3 rounded-xl bg-[#FBF1EC] border border-[#F2DED4] flex items-center justify-between gap-2"
      >
        <p class="text-xs text-[#c17a4f]">
          ⚠️ กรุณายืนยันอีเมลเพื่อเพิ่มร้าน/รีวิวได้
        </p>
        <button
          class="text-xs font-medium text-[#6E8F72] hover:underline shrink-0"
        ...
        <template v-else>
          ...
          >
            ...
            <div class="flex items-center gap-3 mt-1">
              <p
                v-if="restaurant.openTime && restaurant.closeTime"
                class="text-xs text-[#a3a79a] font-light"
              >
                🕐 {{ restaurant.openTime }} - {{ restaurant.closeTime }}
              </p>
              <p
                v-if="restaurant.minPrice && restaurant.maxPrice"
                class="text-xs text-[#a3a79a] font-light"
              >
    ...
    <div class="flex-1 relative z-0 min-h-[300px]">
      <ClientOnly>
        ...
        >
          ...
          >
            <div class="font-['Prompt'] w-[200px]">
              <img
                v-if="activeRestaurant.imageUrls?.length"
                :src="activeRestaurant.imageUrls[0]"
                class="w-full h-28 object-cover rounded-t-lg -mt-1 -mx-1"
              ...
              <div class="px-1 pt-2">
                ...
                <p
                  v-if="activeRestaurant.openTime && activeRestaurant.closeTime"
                  class="text-xs text-[#8B9184] mb-1"
                >
                  🕐 {{ activeRestaurant.openTime }} - {{ activeRestaurant.closeTime }}
                </p>
                <p
                  v-if="activeRestaurant.minPrice && activeRestaurant.maxPrice"
                  class="text-xs text-[#8B9184] mb-2"
                >
