...
<template>
  <div class="flex flex-col md:flex-row h-screen w-full overflow-hidden bg-[#F7F8F5] font-['Prompt']">
    <div
      ...
      <div v-if="currentUser && !currentUser.isVerified"
        ...
        <button class="text-xs font-medium text-[#6E8F72] hover:underline shrink-0" @click="handleResendVerification">
          ส่งอีเมลอีกครั้ง
        </button>
      </div>
          ...
          <InfoWindow v-if="activeRestaurant"
            ...
            <div class="font-['Prompt'] w-[200px]">
              <img v-if="activeRestaurant.imageUrls?.length" :src="activeRestaurant.imageUrls[0]"
                class="w-full h-28 object-cover rounded-t-lg -mt-1 -mx-1" style="width: calc(100% + 8px);">
              <div class="px-1 pt-2">
  ...
  <RestaurantReviewsPanel :key="reviewsPanelRestaurant?.id ?? 'none'" :is-open="isReviewsPanelOpen"
    :restaurant-id="reviewsPanelRestaurant?.id" :restaurant-name="reviewsPanelRestaurant?.name"
    @close="closeReviewsPanel" @updated="handleReviewsUpdated" />
</template>
